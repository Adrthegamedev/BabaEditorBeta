...
function findfeatureat(rule1,rule2,rule3,x,y,blockers_)
	local result = {}
				...
				for i,v in ipairs(targets) do
					if (name == v[1]) then
						...
						for c,d in ipairs(blockers) do
							local testing = hasfeature(name,"is",d,unitid,x,y)
...
function getunitswithverb(rule2,ignorethese_)
	local group = {}
	local result = {}
	local ignorethese = ignorethese_ or {}
...
function hasfeature_count(rule1,rule2,rule3,unitid,x,y)
	local result = 0
