...
function text_tuto(text_,x,y,centered_,wrapping_,colour_,wraplimit_)
	...
	for i,v in ipairs(textlines) do
		writetext(v,0,xpos,ypos + 24 * (i - 1),"tutorial",centered,2,true,colour)
	end
	
	return #textlines
end

function button_tuto(func,text_,x,y,w,h)
	local xpos = screenw * 0.5 + f_tilesize * x
	local ypos = screenh * 0.5 + f_tilesize * y
	local text = ""
	...
	createbutton(func,xpos,ypos,2,w,h,text,"tutorial",nil,nil,"tutorial")
end
