...
function editor_defaulthotbar()
	...
	if (unitreference["baba"] ~= nil) then
		MF_addquickbar(1,unitreference["baba"],0)
	end
	
	if (unitreference["text_you"] ~= nil) then
		MF_addquickbar(2,unitreference["text_you"],0)
	end
	
	if (unitreference["flag"] ~= nil) then
		MF_addquickbar(3,unitreference["flag"],0)
	end
	
	if (unitreference["text_win"] ~= nil) then
	...
	if (unitreference["text_stop"] ~= nil) then
		MF_addquickbar(6,unitreference["text_stop"],0)
	end
	
	if (unitreference["text_push"] ~= nil) then
		MF_addquickbar(7,unitreference["text_push"],0)
	end
	
	if (editor2.values[EXTENDEDMODE] == 1) then
		if (unitreference["line"] ~= nil) then
...
function editor_currobjlist_remove(id_,name,dopairs_)
	local dopairs = true
	...
	if dopairs and (obj.pair ~= nil) then
		...
		if (pairname ~= nil) then
			editor_currobjlist_remove(pairid,pairname,false)
		else
