...
function removetile(unitid,x,y,notilemap_,noundo_,nodelete_)
	...
	local noundo = nonundo_ or false
...
function levelname(name,ox_,oy_)
	local ox = ox_ or 0
	local oy = oy_ or 0
...
function objectwordswap(id,dir_)
	...
	elseif (t == 2) and (unittype == "text") and (string.sub(name, 1, 5) == "text_") then
...
function editor_setselectionrect(x,y,w,h,unitlist_,doundo_)
	...
	for a,bunit in ipairs(db) do
		...
		removetile(bunit.fixed,x_,y_,nil,true,true)
	end
	...
	for i=1,w do
		for j=1,h do
			...
			if inbounds(fx,fy,1) then
				...
				if (selectionrect[tileid] ~= nil) and (#selectionrect[tileid] > 0) then
					local data = selectionrect[tileid][1]
...
function editor_trynamechange(object,newname,fixed,objlistid)
	local valid = true
	
	for a,obj in pairs(editor_objlist) do
		if (obj.name == newname) or (obj.name == "text_" .. newname) then
			valid = false
		end
	end
	
	if (newname == "level") or (newname == "text_crash") or (newname == "text_error") or (newname == "crash") or (newname == "error") or (newname == "text_never") or (newname == "never") then
		valid = false
	end
