	...
	if (#jumble > 0) then
		for j=1,totalcombos do
			...
			for i,cjumble in ipairs(jumble) do
				...
				jumblecombo[i] = (jumblecombo[i] + 1) % #cjumble
				...
				--MF_alert("Currently " .. word .. ", " .. subword .. ", " .. prevword .. ", " .. tostring(dir))
...
function findletterwords(word_,wordpos_,subword_,mainbranch_)
	...
	if (#fullwords > 0) then
		for i,v in ipairs(fullwords) do
			...
			if (unitreference[alttext] ~= nil) then
				local realname = unitreference[alttext]
				
				if (realname == "level") and (unitreference[alttext] ~= nil) then
					realname = unitreference[alttext]
				end
