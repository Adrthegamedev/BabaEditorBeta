...
function displaylevelname(name_,level,layer_,group_,x_,y_,absolute_,prefix_)
...
function controlicon_editor(group,name,x,y,buttonid,text_,textdir,bybuttonname_,altname,alttext_,alttext_2)
	...
	if bybuttonname and (gpad ~= nil) and gpad then
		...
		for i,v in ipairs(data) do
			...
			if (name == "a") and (generaldata.strings[BUILD] == "n") then
				alttext2 = langtext("buttons_editor_selection_place",true)
			elseif (name == "b") and (generaldata.strings[BUILD] == "n") then
				alttext2 = langtext("buttons_editor_selection_cancel",true)
			elseif (name == "x") and (generaldata.strings[BUILD] == "n") then
				alttext2 = langtext("buttons_editor_selection_cancel",true)
			elseif (name == "y") and (generaldata.strings[BUILD] == "n") then
...
function getdynamicbuttonwidth(text,default_,maximum_)
	local prunedtext = string.gsub(text, "[$][0-9][,][0-9]", "")
	local hm,invalid = decode(prunedtext)
	local xscale = #hm + 2
...
function wordwrap(text_,wraplimit,nolangtext)
	...
	if (string.len(text) * 8 > wraplimit) then
		...
		for i=1,string.len(text) do
			...
			if ((l == " ") and (string.len(subtext) * 8 > wraplimit * 0.875)) or (l == "^") then
				table.insert(textlines, subtext)
				subtext = ""
			else
				subtext = subtext .. l
				if (string.len(subtext) * 8 > wraplimit * 0.875) then
...
function continuetext(unitid,iconid)
	...
	local slotname = ""
	if (world ~= "baba") and (world ~= "baba_m") and (world ~= "levels") then
