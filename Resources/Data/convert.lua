function conversion(dolevels_)
	...
	for i,v in pairs(features) do
		...
		if (operator == "is") then
			...
			if (getmat(thing) ~= nil) or (thing == "not " .. name) or (thing == "all") or (unitreference[thing] ~= nil) or ((thing == "text") and (unitreference["text_text"] ~= nil)) or (thing == "revert") then
...
function doconvert(data,extrarule_)
	...
	if (style == "convert") then
		...
		if (mat2 == "revert") and (unitid ~= 2) then
			local originalname = ""
			local oldest = 0
			local source = ""
			local baseid = -1
			...
			for i=1,#undobuffer do
				...
				for a,b in ipairs(curr) do
					if (b[1] == "create") and (b[3] == baseingameid) then
						oldest = i
						if (hasfeature(nametotest,"is","not " .. b[2],unitid,x,y) == nil) then
							originalname = b[2]
							source = b[5]
							baseid = b[4]
							break
